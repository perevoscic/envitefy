...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `ui-styling`, `brand`, `design-system`, and `chrome-devtools` skills.
...
3. **Invoke `/ck:ui-styling`** for layout, hierarchy, and visual balance. Use `/ck:frontend-design` as a fallback when needed.
