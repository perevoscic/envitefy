...
After generation, **ALWAYS** ask user about HTML preview via `AskUserQuestion`. If yes, invoke `/ui-styling` for the gallery.
...
2. **Research** — Activate `design-system` and `ui-styling`, then browse Pinterest for references
...
Multi-platform social image design: HTML/CSS → screenshot export. Uses `ui-styling`, `brand`, `design-system`, and `chrome-devtools` skills.
...
4. **Design** — `/ckm:brand` → `/ckm:design-system` → `/ck:ui-styling`; use `/ck:frontend-design` as a fallback; HTML per idea × size
...
**Related Skills:** frontend-design, ui-styling, ai-multimodal, chrome-devtools
