...
- Event opening preference (September 10, 2026): clicking an owned event in My Events opens `?tab=event` and the fullscreen `EventOwnerView`, with Edit, Share and Delete beside the device selector. Default to Desktop on desktop browsers, Mobile on phones, and Tablet on tablet-sized browsers, using the browser viewport rather than a narrow preview container. Remove the redundant Manage your event / Your event page editing panel; Edit goes to the existing event editor. After successful deletion, clear the selected event and return to the main dashboard with My Events open instead of selecting another event. Preserve guest-response dashboards, actual Live Card artwork editing, and explicit saves.

- Event preview device preference (September 10, 2026): use the shared `EventPreviewViewport` for opened event, Live Card, and Concierge previews. Show icon buttons for Desktop, iPad/tablet, and Mobile with a selected state, plus Close. Resize a real iframe viewport so breakpoints and fixed artwork match the selected device; scale it to fit the available screen. Keep the app sidebar and embedded owner controls out of previews; the owner event view supplies Edit, Share and Delete in its outer toolbar. Keep unsaved preview artwork in memory.
- Fullscreen preview navigation preference (September 10, 2026): opening a preview hides both desktop sidebar and mobile navigation and removes their reserved spacing. Use the shared preview chrome suppression, restore navigation when the last preview closes, and leave the inline event-workspace preview and unsaved editor state intact.
