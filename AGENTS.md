...
- **Behavior**: The recipient email must already belong to an existing Envitefy user. The route now fails closed when the share row cannot be persisted, instead of silently sending an email-only invite that would never appear under Home or Invited Events.
- **Output**: `{ ok: true, share }`.
- **Errors**: Returns HTTP `409` / `{ error, code: "recipient_user_not_found" }` when the recipient email does not map to an existing account. Returns HTTP `400` / `{ error, code: "cannot_share_to_self" }` when the owner tries to share to their own account.
...
- **Behavior**:
  ...
  - Flyer/import saves created by the callback are persisted into history as invited-scanned rows (`data.ownership="invited"`, `data.invitedFromScan=true`, `createdVia: "ocr"`), so they appear under `Invited Events` instead of `My events`.
...
- **Notes**:
  ...
  - `POST /api/history` preserves whatever ownership flags the caller sends. Only flows that explicitly persist `data.ownership="invited"` plus `data.invitedFromScan=true` are treated as invited-scanned rows. OCR-created owner flows (for example discovery/customizer saves) remain under `My events` unless they explicitly mark themselves as invited-scanned.
