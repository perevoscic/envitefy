...
const out = path.resolve(__dirname, '../../output/social-links-review');
...
(async () => {
  ...
  try {
    for (const viewport of [{width:1440,height:1000},{width:375,height:812},{width:812,height:375}]) {
      ...
      const links = await social.getByRole('link').evaluateAll(nodes => nodes.map(a => ({href:a.href,label:a.getAttribute('aria-label'),target:a.target,rel:a.rel,width:a.getBoundingClientRect().width,height:a.getBoundingClientRect().height})));
    ...
    for (const viewport of [{width:1440,height:1000},{width:375,height:812},{width:812,height:375}]) {
      ...
      await context.route('https://www.instagram.com/**',route=>route.fulfill({status:200,contentType:'text/html',body:'<title>Social destination preview</title>'}));
      const popupPromise=page.waitForEvent('popup');
      await social.getByRole('link').first().click();
      const popup=await popupPromise;
      await popup.waitForLoadState();
      assert.equal(popup.url(),'https://www.instagram.com/envitefy/');
      await popup.close();
      await profile.click().catch(async error=>{await page.screenshot({path:path.join(out,'profile-error.png')});console.log((await page.locator('body').innerText()).slice(-2200));throw error;});
