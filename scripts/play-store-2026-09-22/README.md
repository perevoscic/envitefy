...
Editable scripts live here. All generated files described below live in the Git-ignored `output/play-store-2026-09-22/` folder at the repository root. Install the repository dependencies, Playwright Chromium and FFmpeg before rendering.
...
```powershell
node scripts/play-store-2026-09-22/capture-app.cjs
node scripts/play-store-2026-09-22/render.cjs
```

`render.cjs` is the editable design and wording source; `copy-and-layout.json` is a generated snapshot of that wording. The paired WebP proofs are encoded directly from browser PNG buffers using FFmpeg libwebp, quality 85, compression level 6. Intermediate PNG buffers are not written to disk. Final PNGs are retained as the specifically requested Google Play delivery format. ZIP packaging is a separate manual step; the renderer does not rebuild existing archives.
