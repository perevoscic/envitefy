...
export const createSignupSlot = (overrides: Partial<SignupFormSlot> = {}): SignupFormSlot => ({
  id: generateSignupId(),
...
export const createSignupSection = (
  ...
  slots: overrides.slots && overrides.slots.length > 0 ? overrides.slots : [createSignupSlot()],
});
...
const sanitizeSections = (sections: SignupFormSection[]): SignupFormSection[] => {
  return sections
    .map((section) => {
      ...
      if (!slots.length && section.kind !== "info") return null;
      const builtSection: SignupFormSection = {
        id: section.id || generateSignupId(),
        title: normalizedTitle,
        description: normalizedDescription,
        slots,
        ...(section.kind === "info" ? { kind: "info" as const } : {}),
        ...
        ...(typeof section.unitLabel === "string"
          ? { unitLabel: section.unitLabel.trim().slice(0, 40) }
          : {}),
        maxSelectionsPerPerson:
          typeof section.maxSelectionsPerPerson === "number" &&
          Number.isFinite(section.maxSelectionsPerPerson)
            ? Math.max(1, Math.min(50, Math.round(section.maxSelectionsPerPerson)))
            : null,
        ...(typeof section.maxQuantityPerSlot === "number" &&
        Number.isFinite(section.maxQuantityPerSlot)
          ? {
              maxQuantityPerSlot: Math.max(1, Math.min(50, Math.round(section.maxQuantityPerSlot))),
            }
          : {}),
      };
      return builtSection;
...
const sanitizeSettings = (settings: SignupFormSettings): SignupFormSettings => {
  ...
  return {
    ...
    // Older forms used the guest limit for quantities. Preserve that quantity limit on read.
    maxQuantityPerSlot:
      typeof settings.maxQuantityPerSlot === "number" &&
      Number.isFinite(settings.maxQuantityPerSlot)
        ? Math.max(1, Math.min(50, Math.round(settings.maxQuantityPerSlot)))
        : maxGuests,
    collectGuestCount: Boolean(settings.collectGuestCount),
...
export const sanitizeSignupForm = (form: SignupForm): SignupForm => {
  const presentation = {
    ...
    ...(typeof form.boardTitle === "string"
      ...
      : {}),
    ...(typeof form.boardDescription === "string"
    ...
    locationMode:
      form.locationMode === "online" || form.locationMode === "tba"
        ...
        : ("in-person" as const),
    revision:
      typeof form.revision === "number" && Number.isFinite(form.revision)
        ? Math.max(0, Math.floor(form.revision))
        : 0,
  ...
  const responses: SignupResponse[] = Array.isArray(form.responses) ? form.responses : [];
  const questions = sanitizeQuestions(form.questions || []);
  const header: SignupFormHeader | null = (() => {
    const raw = form.header || null;
    if (!raw || typeof raw !== "object") return null;
    const backgroundColor =
      typeof (raw as any).backgroundColor === "string" && (raw as any).backgroundColor.trim()
        ? (raw as any).backgroundColor.trim()
        : null;
    const backgroundCss =
      typeof (raw as any).backgroundCss === "string" && (raw as any).backgroundCss.trim()
    ...
    const templateId =
      typeof (raw as any).templateId === "string" ? ((raw as any).templateId as string) : null;
    const themeId =
      typeof (raw as any).themeId === "string" ? ((raw as any).themeId as string) : null;
    const textColor1 =
      typeof (raw as any).textColor1 === "string" ? ((raw as any).textColor1 as string) : null;
    const textColor2 =
      typeof (raw as any).textColor2 === "string" ? ((raw as any).textColor2 as string) : null;
    const buttonColor =
      typeof (raw as any).buttonColor === "string" ? ((raw as any).buttonColor as string) : null;
    const buttonTextColor =
      typeof (raw as any).buttonTextColor === "string"
        ? ((raw as any).buttonTextColor as string)
        : null;
  ...
  if (!sections.length) {
    return {
      ...
      venue: (form as any).venue?.trim
        ? ((form as any).venue as string).trim() || null
        : ((form as any).venue ?? null),
      location: (form as any).location?.trim
        ? ((form as any).location as string).trim() || null
        : ((form as any).location ?? null),
      room: (form as any).room?.trim
        ? ((form as any).room as string).trim() || null
        : ((form as any).room ?? null),
      parkingInfo: (form as any).parkingInfo?.trim
        ? ((form as any).parkingInfo as string).trim() || null
        : ((form as any).parkingInfo ?? null),
      ...
      start: (form as any).start?.trim
        ? ((form as any).start as string).trim() || null
        : ((form as any).start ?? null),
      end: (form as any).end?.trim
        ? ((form as any).end as string).trim() || null
        : ((form as any).end ?? null),
      setupTime: (form as any).setupTime?.trim
        ? ((form as any).setupTime as string).trim() || null
        : ((form as any).setupTime ?? null),
      earliestDropoff:
      ...
      timezone: (form as any).timezone?.trim
        ? ((form as any).timezone as string).trim() || null
        : ((form as any).timezone ?? null),
      allDay: typeof (form as any).allDay === "boolean" ? Boolean((form as any).allDay) : null,
      gradeOrAge: (form as any).gradeOrAge?.trim
        ? ((form as any).gradeOrAge as string).trim() || null
        : ((form as any).gradeOrAge ?? null),
      audience: Array.isArray((form as any).audience)
        ? ((form as any).audience as any[])
      ...
      themeImage: (form as any).themeImage?.trim
        ? ((form as any).themeImage as string).trim() || null
        : ((form as any).themeImage ?? null),
      colorStory: (form as any).colorStory?.trim
        ? ((form as any).colorStory as string).trim() || null
        : ((form as any).colorStory ?? null),
      ...
      safetyNotes: (form as any).safetyNotes?.trim
        ? ((form as any).safetyNotes as string).trim() || null
        : ((form as any).safetyNotes ?? null),
      requirements: (form as any).requirements?.trim
        ? ((form as any).requirements as string).trim() || null
        : ((form as any).requirements ?? null),
...
const countForSlotByStatus = (
  ...
  const total = form.availability?.find(
    (entry) => entry.sectionId === sectionId && entry.slotId === slotId,
  );
  if (total && (status === "confirmed" || status === "waitlisted")) {
    const excluded = excludeResponseId
      ? form.responses
          .find((response) => response.id === excludeResponseId && response.status === status)
          ?.slots.find((slot) => slot.sectionId === sectionId && slot.slotId === slotId)
          ?.quantity || 0
      : 0;
...
export const countConfirmedForSlot = (
  ...
  slotId: string,
  excludeResponseId?: string | null,
): number => countForSlotByStatus(form, sectionId, slotId, "confirmed", excludeResponseId);
...
export const remainingCapacityForSlot = (
  ...
  const confirmed = countConfirmedForSlot(form, sectionId, slotId, excludeResponseId);
...
export const findSignupResponseForUser = (
  ...
  if (userId) {
    const byUserId =
      form.responses.find(
        (response) => response.userId === userId && response.status !== "cancelled",
      ) ?? null;
  ...
  for (const response of active) {
    ...
    if (canConfirm) {
      overrides.set(response.id, {
        status: "confirmed",
        updatedAt: response.status === "confirmed" ? response.updatedAt : nowIso,
      });
      ...
      overrides.set(response.id, {
        status: "waitlisted",
        updatedAt: response.status === "waitlisted" ? response.updatedAt : nowIso,
      });
    }
  }
