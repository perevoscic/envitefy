...
export function getAuthOptions(): NextAuthOptions {
  ...
  return {
    debug: false,
    secret, // NextAuth will require it in prod at runtime
    providers: [
      CredentialsProvider({
        ...
        async authorize(credentials) {
          try {
            ...
            if (!ok) return null;
            return { id: user.id, email: user.email, name: [user.first_name, user.last_name].filter(Boolean).join(" ") || user.email } as any;
          } catch {
            return null;
          }
        },
