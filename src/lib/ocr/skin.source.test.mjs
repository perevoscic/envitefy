...
test("ocr pipeline adds provider-aware ocrSkin for invite-like OCR categories", () => {
  ...
  assert.match(source, /let ocrSkin: Awaited<ReturnType<typeof inferOcrSkinSelection>> = null;/);
  ...
  assert.match(source, /enableSkinInference && isOcrInviteCategory\(ocrSkinCategory\)/);
