export function buildEventExtractionPrompt(todayIso: string) {
  ...
  • Food truck/vendor/school visit flyers: preserve the main vendor headline plus the school/place in title when visible (e.g. "<Vendor/Event> — <School/Place>"). Put the school/place name in venueName, not address. Menu item prices belong in ocrFacts with label "Menu Prices"; flavor lists belong in ocrFacts with label "Flavors". Do not put prices or flavor names in address/location/venueName. Do not create RSVP fields unless RSVP/respond wording with a real contact method is printed.
  ...
  For food truck/vendor/school visit flyers, preserve title/date/time/place, keep menu prices and flavors in ocrFacts, and leave RSVP fields null unless the flyer explicitly prints an RSVP/respond contact.
