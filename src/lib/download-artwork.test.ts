...
test("Download saves the accepted image bytes with the real media extension", async (t) => {
  const bytes = new Uint8Array([1, 2, 3, 4]);
  const events: string[] = [];
  const anchor = { href: "", download: "", style: { display: "" }, click() { events.push("click"); }, remove() { events.push("remove"); } };
  t.mock.method(globalThis, "fetch", async (url) => { assert.equal(url, "/accepted.webp"); return new Response(bytes, { headers: { "Content-Type": "image/webp" } }); });
  ...
  await downloadArtwork("/accepted.webp", "September 23 workshop");
  assert.equal(anchor.download, "September-23-workshop.webp");
