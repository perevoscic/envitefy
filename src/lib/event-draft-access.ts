...
export function isEventDraft(value: unknown): boolean {
  ...
  return [data.status, data.draftStatus].some((status) => typeof status === "string" && status.trim().toLowerCase() === "draft");
}
export function canReadEventDraft(data: unknown, ownerId: string | null | undefined, viewerId?: string | null): boolean {
