...
test("generated title artwork is reused verbatim and only wording changes require new lettering", () => {
  ...
  assert.equal(
    sharedCardArtworkUrl({ ...source, title: "Livia is turning 11" }),
    "/background.webp",
  );
  ...
  assert.equal(
    hasGeneratedCardHeadline({
      ...source,
      eventDetails: { startTime: "19:00", location: "New venue" },
    }),
    true,
  );
  assert.deepEqual(
    layoutSharedCard(source.sharedDesign, sharedCardContent(source), "live_card", measure, true)
      .lines,
    [],
  );
  assert.equal(
    readSharedCardDesign({ ...design, headline: { ...headline, imageUrl: "javascript:alert(1)" } })
      ?.headline,
    undefined,
  );
});
...
test("downloads contain logistics, contacts and QR links, never Overview-only messages", () => {
  const content = sharedCardContent({
    title: "Livia is turning 10",
    publicUrl: "https://envitefy.com/event/livia",
    eventDetails: {
      detailsDescription: "Overview message only",
      guestInstructions: ["Bring a jacket"],
      eventDate: "2026-09-26",
      startTime: "16:00",
      venueName: "AMC",
      location: "465 Grand Blvd",
      rsvpEnabled: true,
      rsvpName: "Mia",
      rsvpContact: "mia@example.com · 555-123-4567",
      rsvpUrl: "https://rsvp.example.com/reply?id=123",
  ...
  assert.ok(
    layout.lines.every((line) => line.y >= 790),
    "facts never cover generated lettering",
  );
  assert.equal(
    invitationLinks({
      ...
      eventDetails: {
        rsvpEnabled: false,
        rsvpUrl: "https://rsvp.example.com",
        registryLink: "http://localhost/foo",
      },
    }).length,
    0,
  );
