...
async function saveCalendarSync(
  eventId: string,
  claimToken: string,
  patch: { calendarSync: JsonRecord },
): Promise<void> {
  // A worker whose lease expired cannot overwrite a newer retry's result.
  await query(
    `UPDATE event_history SET data = jsonb_set(data, '{calendarSync}', $3::jsonb)
     WHERE id = $1 AND data->'calendarSync'->>'claimToken' = $2`,
    [eventId, claimToken, JSON.stringify(patch.calendarSync)],
  );
}
...
async function persistCalendarSync(params: {
  ...
  await saveCalendarSync(params.eventId, params.claimToken, {
    calendarSync: {
      ...existing,
      status: params.status,
...
export async function syncSavedEventToCalendar({
  eventId,
  userId,
  email,
  origin,
  ...
  if (!provider || !refreshToken) {
    ...
    await saveCalendarSync(eventId, claimToken, {
      calendarSync: {
        ...existingSync,
  ...
  const flyerSourceUrl = hasFlyer
    ? new URL(`/media/events/${mediaIdentifier}/thumbnail?variant=attachment`, origin).toString()
    : "";
  const flyerPreviewUrl = hasFlyer
    ? new URL(`/media/events/${mediaIdentifier}/thumbnail?variant=thumbnail`, origin).toString()
    : "";
