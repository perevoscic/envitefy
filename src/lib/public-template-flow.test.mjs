...
function loadTs(relative, mocks = {}, cache = new Map()) {
  ...
  const resolve = (name) => {
    ...
    const resolved = [base, ...[".ts", ".tsx", ".mjs", ".js", ".json"].map((ext) => base + ext)].find((candidate) => existsSync(candidate));
...
function draft() {
  return { version: 1, id: draftId, category: "weddings", templateId: catalog.getPublicTemplates("weddings")[0].id, updatedAt: Date.now(), snapshot: { data: { partner1: "Casey", partner2: "Taylor", images: { hero: "blob:photo" }, registry: [{ label: "Registry", url: "https://example.com/gifts" }] }, activeView: "images" }, assets: { "blob:photo": new Blob(["photo bytes"], { type: "image/png" }) }, pendingSave: true };
}
...
test("all nine catalogs expose unique valid template/editor identities and existing artwork", () => {
  ...
  for (const category of categories.TEMPLATE_CATEGORIES) {
    ...
    for (const template of templates) {
      ...
      if (template.heroImage.startsWith("/")) assert.ok(existsSync(path.join(root, "public", template.heroImage)), template.heroImage);
    }
  ...
  for (const value of ["/admin/templates", "/weddings/templates/a/customize/delete", "/weddings/templates/a", "/weddings/templates/a/../customize"]) assert.equal(categories.isPublicTemplatePath(value), false);
});

test("explicit drafts are owner-only; published and legacy records retain existing access", () => {
  for (const data of [{ status: "draft" }, { draftStatus: "draft" }, { status: " DRAFT " }, { status: "published", draftStatus: "draft" }]) {
  ...
  for (const data of [{}, { status: "published" }, null]) assert.equal(access.canReadEventDraft(data, "owner"), true);
  assert.equal(access.isClientDraftId(draftId), true);
  for (const value of ["../event", "https://example.com", null, "event-1"]) assert.equal(access.isClientDraftId(value), false);
});

test("guests cannot trigger handoff uploads or history mutations", async () => {
  let requests = 0;
  await assert.rejects(handoff.saveTemplateDraftToAccount({ draft: draft(), payload: { title: "Our wedding", data: {} }, category: "weddings", templateId: "gilded-wedding", status: "draft", authenticated: false, remoteMedia: {}, request: async () => { requests++; return json({}); } }), /Sign in/);
...
test("authentication saves a complete private draft with uploaded photos exactly once", async () => {
  ...
  const request = async (url, options) => {
    ...
    if (url === "/api/templates/media") { assert.equal(await options.body.get("file").text(), "photo bytes"); return json({ url: "https://media.example/photo.webp" }); }
    ...
    assert.equal(body.data.templateEditor.snapshot.data.images.hero, "https://media.example/photo.webp");
    assert.ok(!options.body.includes("blob:"));
    return json({ id: current.id, ...body });
  };
  const options = { draft: current, payload: { title: "Our wedding", data: current.snapshot.data }, category: "weddings", templateId: current.templateId, status: "draft", authenticated: true, remoteMedia, request };
  assert.equal(await handoff.saveTemplateDraftToAccount(options), current.id);
  await handoff.saveTemplateDraftToAccount(options);
  assert.deepEqual(requests.map(({ url, options }) => [url, options.method]), [["/api/templates/media", "POST"], ["/api/history", "POST"], [`/api/history/${current.id}`, "PATCH"]]);
...
test("failed uploads retain browser photos and remain retryable", async () => {
  const current = draft();
  let historyCalls = 0;
  await assert.rejects(handoff.saveTemplateDraftToAccount({ draft: current, payload: { title: "Wedding", data: current.snapshot.data }, category: "weddings", templateId: current.templateId, status: "draft", authenticated: true, remoteMedia: {}, request: async (url) => { if (url === "/api/history") historyCalls++; return json({ error: "Upload failed" }, 503); } }), /Upload failed/);
...
test("lost create responses recover the same identity and apply newer edits via owned update", async () => {
  ...
  const request = async (url, options) => {
    ...
    if (url === "/api/history") {
      posts++;
      if (!saved) { saved = { id: current.id, ...body }; throw new Error("Network response lost"); }
      return json(saved);
    }
    patches++; saved = { id: current.id, ...body }; return json(saved);
  };
  const options = { draft: current, payload: { title: "Wedding", data: current.snapshot.data }, category: "weddings", templateId: current.templateId, status: "draft", authenticated: true, remoteMedia, request };
  ...
  await handoff.saveTemplateDraftToAccount(options);
  assert.equal(posts, 2); assert.equal(patches, 1);
...
function memoryIndexedDb(rows = [], failure = false) {
  ...
  return { records, open() {
    const request = {};
    queueMicrotask(() => {
      if (failure) { request.error = new Error("Quota exceeded"); request.onerror(); return; }
      request.result = { close() {}, transaction() {
        const tx = {};
        const pending = new Map(records);
        const complete = () => setTimeout(() => { records.clear(); for (const [key, value] of pending) records.set(key, value); tx.oncomplete?.(); }, 0);
        tx.objectStore = () => ({
          put(value) { pending.set(value.id, structuredClone(value)); complete(); },
          delete(id) { pending.delete(id); complete(); },
          getAll() { const query = {}; queueMicrotask(() => { query.result = structuredClone([...pending.values()]); query.onsuccess(); complete(); }); return query; },
        });
        return tx;
      } };
      request.onsuccess();
...
test("IndexedDB roundtrip retains photos, section, identity and latest category draft; expiry is seven days", async () => {
  ...
  try {
    ...
    const current = draft(); await storage.writeTemplateDraft(current);
...
test("storage errors are surfaced, never reported as successful retention", async () => {
  const original = globalThis.indexedDB;
  try { globalThis.indexedDB = memoryIndexedDb([], true); await assert.rejects(storage.writeTemplateDraft(draft()), /Quota/); await assert.rejects(storage.readTemplateDraft("weddings"), /Quota/); }
  finally { globalThis.indexedDB = original; }
});
...
test("history POST requires authentication and returns same-owner retries without replacing data", async () => {
  ...
  const route = loadTs("src/app/api/history/route.ts", {
    ...
    "@/lib/db": { getEventHistoryById: async () => saved, insertEventHistory: async (params) => { inserts++; saved = { id: params.clientDraftId, user_id: params.userId, title: params.title, data: params.data }; return saved; } },
  ...
  const request = (title = "Original") => new Request("https://envitefy.test/api/history", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clientDraftId: draftId, title, data: { status: "draft" } }) });
