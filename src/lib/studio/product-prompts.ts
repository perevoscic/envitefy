...
export function buildProductCopyPrompt(
  ...
  return [
    ...
    "For a Live Card, write description as a short, informative guest overview in natural sentences: where and when to meet, the supplied activity or movie title, and what happens afterward at each supplied stop. Keep the sequence clear and preserve any supplied stop times. Do not reduce the plan to venue labels or repeat the birthday/title introduction. Only use PUBLIC_FACTS; never turn a movie theme or other PRIVATE_DIRECTION into a scheduled activity.",
    "Before returning Live Card copy, check description against every supplied public activity, guest instruction and later stop, not only the title or existing description. Retain a named movie as a named movie, its theater and meeting time, then the named restaurant and meal when supplied. 'Movie at the theater, then dinner' is incomplete when more specific facts were provided. Do not drop facts to meet a brevity preference; keep absent facts empty and keep private visual direction out of guest copy.",
