...
test("studio image finish preset registry covers every category with expanded premium presets", () => {
  ...
  assert.match(source, /"Open House":\s*\[/);
  ...
  assert.match(source, /label: "Luxury Listing"/);
  assert.match(source, /label: "Architectural Editorial"/);
  assert.match(source, /label: "Magazine Feature"/);
