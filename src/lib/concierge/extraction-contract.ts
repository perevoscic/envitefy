...
export const CONCIERGE_EXTRACTION_INSTRUCTION = [
  ...
  "A stated activity is a public event fact: 'we are going to watch [movie title]' supplies what guests will watch, not just a design theme. Preserve the supplied movie/activity name, meeting venue and time, every later stop, and the sequence (for example movie, then dinner). Keep later venues and their supplied times in additionalLocations. The application retains sourced activity wording in fallbackDraft.publicContent; use those items when writing the Overview and never discard them because previewCopy.body is a generic birthday introduction. A movie mentioned only as an artwork theme does not establish a screening.",
  ...
  "Making the Overview clearer, more informative or more detailed is a wording revision: return previewCopy with an informative body. Explain where and when guests meet, what they will do including any supplied movie title, and what happens afterward. Before returning, compare the body against the current publicContent, venue, timeText and additionalLocations: every supplied activity, named place, stop time and ordering must be represented. Use full natural sentences; 'Movie at [venue]' and 'Dinner at [restaurant]' alone omit useful details. Never ask the host to repeat facts already supplied, invent missing details, or restore superseded facts from older messages.",
].join("\n");
