...
export async function resolveConciergeWeatherContextFromDraft(params: {
  ...
  const eventIso = eventIsoFromDraft(params.draft);
  if (!location || !eventIso) return buildMissingDetailsContext(location, eventIso);
  ...
  return weatherContext({
    ...
    checkedAt: new Date().toISOString(),
    source: "weatherapi",
    message: `The forecast near ${location} for the event time is ${message}`,
  });
}
...
export async function resolveConciergeWeatherContextFromEvent(params: {
  ...
  const eventIso = eventIsoFromData(params.eventData);
  if (!location || !eventIso) return buildMissingDetailsContext(location, eventIso);
