import assert from "node:assert/strict";
import fs from "node:fs";
import path from "node:path";
import test from "node:test";

const repoRoot = process.cwd();

const readSource = (relativePath) =>
  fs.readFileSync(path.join(repoRoot, relativePath), "utf8");

test("feature visibility is clamped to launched template keys", () => {
  const source = readSource("src/config/feature-visibility.ts");
  const settingsPage = readSource("src/app/settings/page.tsx");

  assert.match(
    source,
    /export const ENABLED_TEMPLATE_KEYS: TemplateKey\[] = \[\s*"birthdays",\s*"weddings",\s*"anniversaries",\s*"baby_showers",\s*"gender_reveal",\s*"sport_events",\s*"gymnastics",\s*"football_season",\s*"general",\s*\];/s
  );
  assert.match(
    source,
    /export const TEMPLATE_DEFINITIONS: TemplateDef\[] = ALL_TEMPLATE_DEFINITIONS\.filter\(\s*\(definition\) => ENABLED_TEMPLATE_KEY_SET\.has\(definition\.key\)\s*\)/s
  );
  assert.match(
    source,
    /export const TEMPLATE_KEYS: TemplateKey\[] = \[\.\.\.ENABLED_TEMPLATE_KEYS\];/
  );
  assert.match(
    source,
    /const SPORTS_KEYS: TemplateKey\[] = \["gymnastics", "sport_events", "football_season"\];/
  );
  assert.match(source, /defaultCreateIntent: string \| null;/);
  assert.match(source, /sportPreferences: SportPreferences;/);
  assert.match(
    source,
    /hasExplicitTemplateKeys \? normalizedKeys : \[\.\.\.preset\]/,
  );
  assert.match(settingsPage, /TEMPLATE_DEFINITIONS\.filter\(\(template\) => !isSportsKey\(template\.key\)\)/);
  assert.match(settingsPage, /Your sports/);
  assert.match(settingsPage, /Reset personalization/);
});
