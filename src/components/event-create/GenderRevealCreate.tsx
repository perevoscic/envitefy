...
import GenderRevealTemplate from "@/components/event-templates/GenderRevealTemplate";
import {
...
export default function GenderRevealCreate({
  defaultDate,
  editEventId,
  variant,
  templateId,
  ...
  const variantLabel = "Gender Reveal";
  ...
  const selectedTemplate =
    useMemo<GenderRevealTemplateDefinition | null>(() => {
      if (!templateIdState) return null;
      return (
        genderRevealTemplateCatalog.find((tpl) => tpl.id === templateIdState) ??
        null
      );
    }, [templateIdState]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!row || !row.data || cancelled) return;
        const data = row.data as any;
        // Title
        setTitle(typeof row.title === "string" ? row.title : "Event");
        // When
        try {
        ...
        // Header customization + title style
        ...
        // Repeat
        try {
          const rf = (data.repeatFrequency as string) || "weekly";
          setRepeat(Boolean(data.repeat) || Boolean(data.recurrence));
          if (rf === "weekly" || rf === "monthly" || rf === "yearly")
            setRepeatFrequency(rf as any);
      ...
      prev.map((entry: any) => {
        ...
        if (field === "url") {
          if (!trimmed.trim()) {
            ...
            const v = validateRegistryUrl(trimmed);
            next.error = v.ok ? null : v.error || null;
            next.detectedLabel = v.ok && v.brand ? v.brand.defaultLabel : null;
            if (v.ok && v.brand && (!entry.label || !entry.label.trim()))
              next.label = v.brand.defaultLabel;
          }
        }
    ...
    try {
      ...
      if (whenDate) {
        if (fullDay) {
          ...
          const e2 = new Date(`${endBase}T${endTime || "10:00"}:00`);
          startISO = s.toISOString();
          endISO = e2.toISOString();
        }
      }
      ...
      <form onSubmit={submit} className="space-y-6">
        {selectedTemplate && (
          <section className="rounded-2xl border border-dashed border-[#E4CDB9] bg-[#FFF9F6] px-4 py-3 sm:flex sm:items-center sm:justify-between">
            ...
            <button
              type="button"
              onClick={() => router.push("/event/gender-reveal")}
        ...
        {(() => {
          ...
          return <GenderRevealTemplate editor={editor} />;
        })()}
