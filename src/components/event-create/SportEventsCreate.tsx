...
export default function SportEventsCreate({ defaultDate }: Props) {
  ...
  const handleAttachmentOnlyChange = async (
    ...
    try {
      const dataUrl = await readFileAsDataUrl(file);
      setAttachment({ name: file.name, type: file.type, dataUrl });
      // do not touch preview/colors so header doesn't change
    } catch {
      setAttachment(null);
      e.target.value = "";
    }
  ...
  const eventTheme = getEventTheme("Sport Events");
