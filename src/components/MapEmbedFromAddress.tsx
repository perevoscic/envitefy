...
import {
  ...
  buildGoogleMapsEmbedUrlFromAddress,
} from "@/lib/geocoding";

type MapEmbedFromAddressProps = {
  address: string;
  className?: string;
};

/**
 * MapEmbed component that displays a Google Maps view using just an address string
 * Google Maps handles the geocoding automatically
 */
export default function MapEmbedFromAddress({
  address,
  className = "",
}: MapEmbedFromAddressProps) {
  ...
  const searchUrl = buildGoogleMapsSearchUrlFromAddress(address);
  const embedUrl = mapKey
    ? buildGoogleMapsEmbedUrlFromAddress(address, mapKey)
    : null;
