...
export default function WeddingTemplateRunway({
  ...
  return (
    <div
      role="group"
      aria-label="Four Envitefy wedding website designs"
      className="relative isolate grid grid-cols-2 gap-3 sm:gap-4 xl:block xl:h-[320px]"
    >
      {designs.map((design, index) => (
        <article
          key={design.id}
          className={`group relative min-w-0 rounded-[1rem] border border-[#e3d8c8] bg-[#fffdf8] p-1.5 text-[#2b241f] shadow-[0_20px_50px_rgba(91,70,44,0.14)] transition duration-300 ease-out hover:z-20 focus-within:z-20 xl:absolute xl:w-[43%] xl:hover:-translate-y-2 xl:hover:scale-[1.025] xl:focus-within:-translate-y-2 xl:focus-within:scale-[1.025] motion-reduce:transition-none ${RUNWAY_POSITIONS[index] || ""}`}
        >
          <Link
            href={getHref(design.id)}
            aria-label={`View the ${design.name} wedding website design`}
            className="absolute inset-0 z-20 rounded-[1rem] outline-none focus-visible:ring-2 focus-visible:ring-[#84653e] focus-visible:ring-offset-2 focus-visible:ring-offset-[#fffcf7]"
          >
            <span className="sr-only">View {design.name}</span>
          </Link>
          <WeddingDesignPreview design={design} compact className="rounded-[0.7rem]" />
