...
import { parseEventGuestDate, type EventGuestPlanning, getEventEndLocal } from "@/lib/event-guest-planning";
...
const eventsSection = {
  ...
  renderPreview: ({
    ...
    const fmtDate = (d?: string) =>
      d
        ? parseEventGuestDate(d).toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
          })
        : "";
  ...
  return function SimpleCustomizePage() {
    ...
    const renderDetailsEditor = () => (
      ...
      >
        <div className="space-y-4">
          ...
          <EventGuestPlanningEditor category="cheerleading" value={data.guestPlanning} onChange={(guestPlanning) => setData((prev) => ({ ...prev, guestPlanning }))} />
    ...
    return (
      <div className="relative flex min-h-screen w-full bg-slate-100 overflow-hidden font-sans text-slate-900">
        <div
          {...previewTouchHandlers}
          className="flex-1 min-w-0 relative overflow-y-auto scrollbar-hide bg-[#f0f2f5] flex justify-center"
          style={{
...
<div className="relative w-full h-64 md:h-96">
                ...
                >
                  <EnvitefyEventBranding category="Cheerleading" inverse={isDarkBackground} />
                </footer>
              </div>
            </div>
          </div>
