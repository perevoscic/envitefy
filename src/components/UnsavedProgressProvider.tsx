...
export function useUnsavedProgress(progress: Progress) {
  ...
  useLayoutEffect(
    () => context.register(Symbol("event-progress"), () => current.current),
    [context.register],
  );
  return context;
}
...
export default function UnsavedProgressProvider({ children }: { children: ReactNode }) {
  ...
  const dirtyEntries = useCallback(
    () => [...entries.current.values()].map((read) => read()).filter((entry) => entry.dirty),
    [],
  );
  ...
  const requestLeave = useCallback(
    (navigate: Navigation) => {
      if (bypass.current || !dirtyEntries().length) {
        navigate();
        return;
      }
  ...
  const value = useMemo(
    () => ({ register, requestLeave, allowNavigation }),
    [register, requestLeave, allowNavigation],
  );
    ...
    () =>
      router && {
        ...
        replace: (...args: Parameters<typeof router.replace>) =>
          requestLeave(() => router.replace(...args)),
  ...
  useEffect(() => {
    ...
    const onClick = (event: MouseEvent) => {
      ...
      requestLeave(() => {
        if (target.origin === location.origin && router)
  ...
  return (
    <ProgressContext.Provider value={value}>
      <AppRouterContext.Provider value={guardedRouter}>
        <div className="contents" inert={open ? true : undefined}>
          {children}
        </div>
        <Dialog.Root
          open={open}
        ...
        >
          <Dialog.Portal>
            ...
            <Dialog.Content
              ...
              className="fixed left-1/2 top-1/2 z-[1001] w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-3xl border border-[#e5dcf7] bg-white p-6 text-[#251b3b] shadow-2xl sm:p-8"
