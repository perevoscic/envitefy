...
test("saved concierge drafts stay in chat with a generated product preview", () => {
  ...
  assert.match(chatSurface, /Draft ready/);
  assert.match(chatSurface, /Generated artwork/);
  assert.doesNotMatch(chatSurface, /Guest preview/);
  assert.match(chatSurface, /Edit in chat/);
  assert.match(chatSurface, /Share/);
