...
export async function generateMetadata({ params }: Props) {
  ...
  if (!category) return {};
  return { title: `${category.name} templates | Envitefy`, description: `Browse ${category.name.toLowerCase()} templates and customize your own. Create an account when you’re ready to save and share.`, alternates: { canonical: `/${category.slug}/templates` } };
}
export default async function TemplatesPage({ params }: Props) {
  ...
  return <><SignedOutPageChrome brandHref={`/${category.slug}`} /><main className="pt-24"><PublicTemplateGallery category={category.slug} /></main></>;
}
