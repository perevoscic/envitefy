...
test("service worker does not cache Next.js app chunks", () => {
  ...
  assert.match(source, /const CACHE_NAME = "smd-static-v14";/);
  assert.match(source, /"\/manifest\.webmanifest\?v=v14"/);
  assert.doesNotMatch(source, /"\/manifest\.webmanifest\?v=v13"/);
