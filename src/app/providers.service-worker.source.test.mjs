...
test("service worker does not cache Next.js app chunks", () => {
  ...
  assert.match(source, /const CACHE_NAME = "smd-static-v11";/);
  assert.match(source, /"\/manifest\.webmanifest\?v=v11"/);
  assert.doesNotMatch(source, /"\/manifest\.webmanifest\?v=v10"/);
