...
export default function GenderRevealTemplateCustomizePage() {
  ...
  const resolvedVariation: ResolvedTemplateVariation = useMemo(() => {
    ...
    if (!variation) {
      return {
        ...
        swatches: ["#ffffff"],
        titleColor: "#000000",
        titleFontFamily: "var(--font-poppins)",
        titleWeight: "normal",
        titleAlign: "center",
      };
    }
  ...
  const previewName =
    eventTitle.trim() || template.preview?.coupleName || "Gender Reveal";
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      const payload: any = {
        title: title || eventTitle || "Gender Reveal",
        data: {
        ...
        <div className="rounded-[40px] bg-gradient-to-br from-[#F5E8FF] via-[#E8F5F0] to-[#FFE8F5] p-8 shadow-2xl">
          <div className="space-y-4 text-[#2F2F2F]">
            <p className="text-xs font-semibold uppercase tracking-[0.4em] text-[#9B7ED9]">
              Customize your gender reveal invite
            </p>
            <h1
