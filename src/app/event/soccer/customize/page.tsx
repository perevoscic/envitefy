...
const SOCCER_GOOGLE_FONTS_URL = `https://fonts.googleapis.com/css2?family=${SOCCER_GOOGLE_FONT_FAMILIES.join(
  "&family="
)}&display=swap`;
...
function createSimpleCustomizePage(config: SimpleTemplateConfig) {
  return function SimpleCustomizePage() {
    ...
    // Ensure the soccer template has the same sporty headline fonts available
    useEffect(() => {
      let link =
        document.querySelector<HTMLLinkElement>(
          'link[data-soccer-fonts="true"]'
        ) || null;
      let added = false;
      ...
      if (!link) {
        ...
        link.rel = "stylesheet";
        link.setAttribute("data-soccer-fonts", "true");
        added = true;
      }

      link.href = SOCCER_GOOGLE_FONTS_URL;
    ...
    const selectedFont =
      SOCCER_FONTS.find((f) => f.id === data.fontId) || SOCCER_FONTS[0];
    const selectedSize =
    ...
    const renderDetailsEditor = () => (
      ...
      >
        <div className="space-y-4">
          ...
          <EventGuestPlanningEditor category="soccer" value={data.guestPlanning} onChange={(guestPlanning) => setData((prev) => ({ ...prev, guestPlanning }))} />
          ...
          <div className="w-full min-w-0 mb-12 md:mb-16 transition-all duration-500 ease-in-out">
            ...
            >
              <div className="relative z-10">
                ...
                >
                  <EnvitefyEventBranding category="Soccer" inverse={isDarkBackground} />
                </footer>
              </div>
            </div>
          </div>
