...
export default function CinematicWedding({ theme, event }: Props) {
  ...
  const story = event.story || "From the coffee shops of Brooklyn to the streets of Paris, their journey has been nothing short of cinematic. Join us for the climax of act one as they exchange vows.";
  const galleryImages = event.gallery?.slice(0, 2) || event.photos?.slice(0, 2) || [];
  ...
  return (
    ...
    >
      ...
      <nav className="fixed top-0 w-full z-50 px-8 py-6 flex justify-between items-center mix-blend-difference">
        <div className="text-xs font-bold tracking-[0.3em] uppercase">The Premiere</div>
        <div className="hidden md:flex gap-8 text-xs font-bold tracking-[0.2em] uppercase">
          <a href="#plot" className="hover:opacity-50 transition-opacity">The Plot</a>
          <a href="#set" className="hover:opacity-50 transition-opacity">The Set</a>
          <a href="#cast" className="hover:opacity-50 transition-opacity">The Cast</a>
        </div>
        <a
          href={rsvpUrl}
        ...
        <div className="relative z-10 max-w-4xl">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start md:items-center border-t border-white/20 pt-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold uppercase tracking-widest">{names}</p>
                <p className="text-xs text-gray-500 uppercase tracking-widest">Director's Cut</p>
              </div>
            </div>
            <div className="hidden md:block w-px h-12 bg-white/20"></div>
            <div>
              <p className="text-xs text-gray-400 uppercase tracking-widest mb-1">Release Date</p>
              <p className="text-xl font-bold uppercase tracking-widest">{dateFormatted}</p>
            </div>
          </div>
        </div>
      </header>
      ...
      {/* Content Scroller */}
      <section id="plot" className="py-32 px-8 md:px-24 max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
        <div>
          <h2
          ...
          <p className="text-gray-400 leading-relaxed text-lg mb-8 max-w-md">{story}</p>
        </div>
        <div className="grid grid-cols-2 gap-4">
          {galleryImages.length > 0 ? (
            <>
              <img
                src={galleryImages[0]?.url || galleryImages[0] || "https://images.unsplash.com/photo-1511285560982-1356c11d4606?q=80&w=600"}
                className="w-full aspect-[3/4] object-cover grayscale hover:grayscale-0 transition-all duration-700"
                alt=""
              />
              {galleryImages.length > 1 ? (
                <img
                  src={galleryImages[1]?.url || galleryImages[1] || "https://images.unsplash.com/photo-1623771799757-55030ee08c2a?q=80&w=600"}
      ...
      {/* Production Details */}
      <section id="set" className="py-24 bg-neutral-900 border-y border-neutral-800">
        <div className="max-w-7xl mx-auto px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8 text-center">
            <div className="group p-8 border border-neutral-800 hover:bg-neutral-800 transition-colors">
              ...
              <h3 className="text-xl font-bold uppercase tracking-widest mb-2">The Date</h3>
              <p className="text-gray-400">{event.date || "December 14, 2025"}</p>
            </div>
            <div className="group p-8 border border-neutral-800 hover:bg-neutral-800 transition-colors">
              <Clock className="w-8 h-8 mx-auto mb-6 text-white group-hover:scale-110 transition-transform" />
              <h3 className="text-xl font-bold uppercase tracking-widest mb-2">Showtime</h3>
        ...
        <section id="cast" className="py-24 px-8 md:px-24 max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {event.party.map((member, idx) => (
              <div key={idx} className="border border-neutral-800 p-6 hover:bg-neutral-900 transition-colors">
                <h3 className="text-xl font-bold uppercase tracking-widest mb-2">{member.name}</h3>
      ...
      {/* RSVP */}
      {event.rsvpEnabled && (
        <section id="rsvp" className="min-h-[70vh] flex items-center justify-center px-4 relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-xl w-full text-center">
            ...
            <p className="text-gray-400 mb-12 uppercase tracking-widest text-xs">Limited Capacity Event</p>
