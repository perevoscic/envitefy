...
export default async function EventPage({
  ...
  const themeStyleVars: CSSProperties = {
  ...
  return (
    <main className="max-w-3xl mx-auto px-10 py-14 ipad-gutters pl-[calc(2rem+env(safe-area-inset-left))] pr-[calc(2rem+env(safe-area-inset-right))] pt-[calc(3.5rem+env(safe-area-inset-top))] pb-[calc(3.5rem+env(safe-area-inset-bottom))]">
      {isReadOnly && (
        <div className="mb-6 flex justify-center">
          <img
            ...
            className="block h-28 w-auto dark:hidden"
          />
          <img
      ...
      <div className="event-theme-scope space-y-6" style={themeStyleVars}>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                {!isReadOnly && isOwner && (
                  <div className="flex items-center gap-2 text-sm font-medium">
                    <EventEditModal eventId={row.id} eventData={data} eventTitle={title} />
        ...
        <section
          className={`event-theme-card relative rounded-2xl border px-6 py-6 shadow-sm sm:px-8${detailsExtraPaddingRight}`}
        >
          {data?.thumbnail && (
            <ThumbnailModal src={data.thumbnail as string} alt={`${title} flyer`} />
          )}
          <dl className="grid grid-cols-1 gap-5 text-sm sm:grid-cols-2">
            ...
            <div>
              ...
              <dd className="mt-1 text-base font-semibold">{data?.location || "—"}</dd>
            </div>
            <div>
              <dt className="text-xs font-semibold uppercase tracking-wide opacity-70">
                Category
            ...
            {(rsvpName || rsvpPhone) && (
              <div className="sm:col-span-2 space-y-2">
                ...
                <dd className="space-y-2">
                  {rsvpName && <p className="text-sm font-medium">{rsvpName}</p>}
                  <EventRsvpPrompt
                    rsvpName={rsvpName}
                    rsvpPhone={rsvpPhone}
                    eventTitle={title}
                    ...
                    defaultSmsBody={isSignedIn ? smsBody : null}
                  />
                </dd>
              </div>
            )}
