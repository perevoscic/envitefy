...
const shouldHideInferredOcrEndTime = (
  ...
  if (!payload || !startInput || !endInput || payload.allDay) return false;
  const createdVia =
    typeof payload.createdVia === "string" ? payload.createdVia.trim().toLowerCase() : "";
...
const shouldHideMissingOcrStartTime = (
  ...
  if (!payload || !startInput || payload.allDay) return false;
  const createdVia =
    typeof payload.createdVia === "string" ? payload.createdVia.trim().toLowerCase() : "";
  if (!createdVia.startsWith("ocr")) return false;
...
export default async function EventPage({
  ...
  const endForDisplayLabel =
    hideInferredOcrEndTime || hideMissingOcrStartTime ? null : endForDisplay;
  ...
  const publicStudioDetails = asPlainRecord(asPlainRecord(asPlainRecord(data?.studioCard).invitationData).eventDetails);
  const publicDetails = { ...publicStudioDetails, ...asPlainRecord(data?.eventDetails), ...publicEventRecord };
  const publicLines = publicGuestInstructions({
    guestInstructions: [data?.guestInstructions, publicDetails.guestInstructions].flatMap((value) => Array.isArray(value) ? value.filter((line): line is string => typeof line === "string") : []),
    requiredArtworkLines: [data?.requiredArtworkLines, publicDetails.requiredArtworkLines].flatMap((value) => Array.isArray(value) ? value.filter((line): line is string => typeof line === "string") : []),
    giftNote: firstDisplayString(data?.giftNote, publicDetails.giftNote),
    giftPreferenceNote: firstDisplayString(data?.giftPreferenceNote, publicDetails.giftPreferenceNote),
  });
  const publicTypography = asPlainRecord(publicDetails.pageTypography);
  ...
  const publicSourceSections = Array.isArray(publicEventRecord.sourceSections)
    ? publicEventRecord.sourceSections
        .map((item) => {
          const record = asPlainRecord(item);
          const title = firstDisplayString(record.title, record.label);
          const items = Array.isArray(record.items)
            ? record.items.map((value) => firstDisplayString(value)).filter(Boolean)
            : [];
  ...
  const rsvpNameRaw = rsvpContactSource
    ? rsvpContactSource
        ...
        .replace(
          /\b(?:rsvp|respond|reply)\s+by\s+[A-Z][a-z]+\.?\s+\d{1,2}(?:st|nd|rd|th)?(?:,\s*\d{4})?(?:\s+at)?/gi,
          "",
        )
  ...
  const calendarLinks =
    calendarStartIso
      ? buildCalendarLinks({
          ...
          details: data,
          eventUrl: shareUrl,
          description: [...new Set([(data?.description as string | undefined) || "", ...publicLines].filter(Boolean))].join("\n\n"),
          location: composeGuestLocation(venueText, locationText || (data?.location as string | undefined)),
  ...
  const isScannedPickleballInviteEvent =
    ...
    (String((data as any)?.ocrSkin?.sportKind || "").toLowerCase() === "pickleball" ||
      ...
      ((categoryNormalized === "sport events" ||
        ...
        categoryNormalized === "general events" ||
        categoryNormalized === "general event" ||
        categoryNormalized === "pickleball") &&
        isPickleballOcrSkinCandidate({
          category: categoryRaw,
          title,
          description: [
            (data as any)?.description,
            (data as any)?.goodToKnow,
            (data as any)?.thingsToDo,
          ]
            .filter(Boolean)
            .join(" "),
          activities: (data as any)?.activities,
        })));
  const isScannedFootballInviteEvent =
    categoryNormalized !== "birthdays" &&
    categoryNormalized !== "weddings" &&
    isScannedInviteEvent &&
    isOcrEvent &&
  ...
  const scannedInviteOcrFacts = mergeOcrFacts(
    ...
    buildOcrFacts([
      { label: "Good to Know", value: (data as any)?.goodToKnow },
      { label: "Details", value: (data as any)?.thingsToDo },
      { label: "Dress Code", value: (data as any)?.attire },
      { label: "Host", value: (data as any)?.hostName },
    ]),
  );
  ...
  if (isConciergeEventPageProduct) {
    return renderWithEventPageBackground(
      <ConciergeEventWebsite
        ...
        whenLabel={formatGuestScheduleFromInstants(calendarStartIso, normalizeIso(rawEndIso), typeof data?.timezone === "string" ? data.timezone : "UTC") || publicWhenLine || whenLabel || null}
        ...
        sourceSections={publicSourceSections}
        ...
        guestInstructions={publicLines}
        pageTypography={{ scale: typeof publicTypography.scale === "number" ? publicTypography.scale : 1, contrast: publicTypography.contrast === "high" ? "high" : undefined, foreground: publicTypography.foreground === "dark" ? "dark" : "light" }}
        genderRevealConfig={parseGenderRevealConfig(data)}
        rsvpDeadline={firstDisplayString(data?.rsvpDeadline, asPlainRecord(data?.rsvp).deadline)}
      />,
    );
  }
