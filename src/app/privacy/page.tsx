...
const privacySections = [
  ...
  },
  {
    ...
    body: [
      ...
      "Storage and deletion: Envitefy stores the Google account fields needed for your account, an OAuth refresh token associated with the connected Envitefy account, and limited calendar-sync metadata such as the Google event identifier, link, connection status, and sync result. Envitefy does not make a local copy of your general Google Calendar contents. Disconnecting Google Calendar in Settings deletes this Envitefy account’s stored Google token and stops its calendar sync. It preserves connections belonging to other Envitefy accounts. You can revoke Envitefy’s access in your Google Account settings; that revocation can affect other Envitefy accounts using the same Google account. Account deletion removes connected tokens, subject only to limited backup and legal-retention periods.",
