...
export async function PUT(request: Request) {
  ...
  if (!user.ok) return NextResponse.json({ ok: false, error: "Sign in to save your progress." }, { status: 401 });
  try {
    const body: unknown = await request.json();
    if (!body || typeof body !== "object" || Array.isArray(body))
      return NextResponse.json({ ok: false, error: "Draft progress is required." }, { status: 400 });
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Unable to save your progress." }, { status: 400 });
  }
}
