...
import {
  type EventThemeRequest,
  EventThemeRequestError,
  generateEventTheme,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    input = parseEventThemeRequest(JSON.parse(Buffer.concat(chunks).toString("utf8")));
  } catch (error) {
    return json(
      {
        error:
          error instanceof EventThemeRequestError
            ? error.message
  ...
  try {
    ...
    if (error instanceof EventThemeRequestError) return json({ error: error.message }, 400);
    return json(
      {
