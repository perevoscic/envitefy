...
export async function GET(
  ...
  try {
    ...
    const identity = await resolveEventHistoryIdentityBySlugOrId({ value: awaitedParams.id });
    if (!identity) return new Response("No thumbnail found", { status: 404 });

    const variantParam = String(req.nextUrl.searchParams.get("variant") || "").trim();
    const variant: EventHistoryMediaVariant | null =
      ...
      variantParam === "signup-header"
        ? variantParam
        : null;
    const imageDataUrl = await getEventHistoryMediaDataUrlById(identity.id, variant);
    return buildMediaResponse(imageDataUrl);
  } catch (error) {
    console.error("Public thumbnail route error:", error);
