...
function buildFallbackInvitationData(data: Record<string, unknown>) {
  ...
  return {
    ...
    eventDetails: {
      ...
      calendarStartISO: readFirstString(eventDetailsRaw?.calendarStartISO, data.startISO),
      calendarEndISO: readFirstString(eventDetailsRaw?.calendarEndISO, data.endISO),
      timezone: readFirstString(eventDetailsRaw?.timezone, data.timezone),
      guestInstructions: [eventDetailsRaw?.guestInstructions, data.guestInstructions].flatMap((value) => Array.isArray(value) ? value.filter((line): line is string => typeof line === "string") : []),
      requiredArtworkLines: [eventDetailsRaw?.requiredArtworkLines, data.requiredArtworkLines].flatMap((value) => Array.isArray(value) ? value.filter((line): line is string => typeof line === "string") : []),
      giftNote: readFirstString(eventDetailsRaw?.giftNote, data.giftNote),
      giftPreferenceNote: readFirstString(eventDetailsRaw?.giftPreferenceNote, data.giftPreferenceNote),
      semanticKind: readFirstString(eventDetailsRaw?.semanticKind, data.semanticKind),
      genderReveal: data.genderReveal,
