...
Read the mandatory [brand standard](../../../../../ENVITEFY_BRAND.md). The current target is **en-VITE-fy**, approximately **/ɛnˈvaɪtfaɪ/**, one continuous word without an extra “ih” or “tee” syllable. The shared `scripts/brand-pronunciation.mjs` helper applies the standard's model direction to native dialogue; for ElevenLabs it uses adjacent CMU Arpabet syllable tags (**EH1 N**, **V AY1 T**, **F AY0**, with no spaces between tags) on Flash v2 and the standard IPA in native slash syntax on Eleven v3. These controls specify the target but failed the user’s John-campaign listening check; do not treat tags or a single automated pass as proof of correct sound. The successful John revision instead uses an exact-IPA System.Speech reference followed by stock Jessica voice conversion: scripts/john-brand-local-reference.ps1 and scripts/john-brand-reference-to-jessica.mjs. Its isolated and both final mixed occurrences passed audio review. Use this reference method when direct TTS keeps inserting a syllable, and review each resulting take. The narration pipeline selects a compatible model when necessary. Keep pronunciation metadata out of captions and visible copy. Listen to every generated occurrence before accepting it; model alignment and transcription alone cannot verify its sounds or stress. See [STUDIO-GUIDE.md](../../../../STUDIO-GUIDE.md).
