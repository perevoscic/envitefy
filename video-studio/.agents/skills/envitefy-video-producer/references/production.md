...
Read the mandatory [brand standard](../../../../../ENVITEFY_BRAND.md). The user's September 9 audio reference now governs pronunciation: approximately **in-VY-tih-fy**, **/ɪnˈvaɪtɪfaɪ/**, four connected syllables with VY stress. It supersedes the earlier three-syllable approximation. Keep phonetic metadata out of captions and visible copy; speech recognition's spelling is not the brand spelling.

The shared `scripts/brand-pronunciation.mjs` helper carries this target for native dialogue and compatible TTS controls. Audition generated audio against the recording; correct markup alone is insufficient. When direct TTS drifts, the John campaign's successful method generates a local stock voice with exact IPA, then converts that wholly synthesized sentence into stock Jessica. Scripts: `john-brand-user-reference.ps1`, `john-brand-user-reference-to-jessica.mjs`, and `compare-john-user-pronunciation.mjs`. The original user recording was used for the explicitly authorized Gemini comparison; only the newly synthesized stock reference was sent to ElevenLabs. Both finished John narration mixes were compared directly with the recording. The user then explicitly approved the pronunciation. The durable reference and accepted Jessica line, with hashes and approval, are in assets/brand/audio/pronunciation.json. Reuse the approved take when the script is unchanged.

See [STUDIO-GUIDE.md](../../../../STUDIO-GUIDE.md) and [campaign revision notes](../../../../projects/john-space-disco/revision-user-audio-reference-2026-09-09.md).
