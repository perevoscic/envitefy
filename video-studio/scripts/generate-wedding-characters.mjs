...
const project=path.join(studio,'projects/wedding-characters');
const out=path.join(studio,'public/projects/wedding-characters');
...
async function generate(mode){
 if(!['planner','dancer','crier','planner-wedding','dancer-wedding','crier-wedding','dancer-phone-v2','crier-contact-v3','music'].includes(mode))throw new Error('Invalid shot');
 ...
 if(!state){
  ...
  if(mode.endsWith('-wedding')||mode==='dancer-phone-v2'||mode==='crier-contact-v3'){const prev=await readJson(path.join(project,(mode==='crier-contact-v3'?'crier':mode==='dancer-phone-v2'?'dancer':mode.replace('-wedding',''))+'-job.json'));if(prev?.status!=='completed'){console.log('wedding: waiting for home reference');return;}body.previous_interaction_id=prev.id;}
 ...
 if(!state.id&&state.status!=='completed')throw new Error(`${mode}: inspect ambiguous submission; do not resubmit`);
