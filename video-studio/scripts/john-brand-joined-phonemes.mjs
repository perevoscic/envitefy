...
const requestedModel=process.argv[4]||'eleven_flash_v2';
if(!['eleven_flash_v2','eleven_v3'].includes(requestedModel))throw Error('Unsupported voice model');
const model=speechModelForText(texts[mode],requestedModel);
const body={text:texts[mode].replace(/Envitefy/g,'<phoneme alphabet="cmu-arpabet" ph="EH1 N">En</phoneme><phoneme alphabet="cmu-arpabet" ph="V AY1 T">vite</phoneme><phoneme alphabet="cmu-arpabet" ph="F AY0">fy</phoneme>'),model_id:model,voice_settings:{stability:model==='eleven_v3'?0.5:0.40,similarity_boost:0.75,style:0.22,use_speaker_boost:true,speed:1.08}};
...
const result=await r.json();await fs.writeFile(file,Buffer.from(result.audio_base64,'base64'));await fs.writeFile(`projects/john-space-disco/voice-${mode}-${version}-alignment.json`,JSON.stringify(result.normalized_alignment||result.alignment,null,2));await fs.writeFile(record,JSON.stringify({status:'completed',voice:'Jessica',pronunciationDirection:ENVITEFY_PRONUNCIATION_NOTE,body},null,2));console.log(`Generated ${mode}`);
