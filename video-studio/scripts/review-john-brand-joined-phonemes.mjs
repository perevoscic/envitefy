...
const source='public/projects/john-space-disco/vo-create-v12.mp3';
const resultPath='projects/john-space-disco/brand-standard-voice-v12-audio-review.json';
const requestPath='projects/john-space-disco/brand-standard-voice-v12-audio-review-request.json';
...
if(sha256!=='e64f3139478f70e26a643962cd942d119889db7364985b161cccd9a9ec336dbd')throw Error('The verified synthetic stock-voice file changed.');
const tts=JSON.parse(await fs.readFile('projects/john-space-disco/voice-create-v12-request.json','utf8'));
if(tts.status!=='completed'||tts.voice!=='Jessica'||tts.body.text!=='Bring their birthday ideas to life with <phoneme alphabet="cmu-arpabet" ph="EH1 N">En</phoneme><phoneme alphabet="cmu-arpabet" ph="V AY1 T">vite</phoneme><phoneme alphabet="cmu-arpabet" ph="F AY0">fy</phoneme> Concierge.')throw Error('Synthetic narration provenance mismatch.');
