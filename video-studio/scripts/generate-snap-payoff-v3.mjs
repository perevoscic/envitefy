...
const name='payoff-v3';
const dest=assets+name+'.mp4';
if(await fs.stat(dest).catch(()=>null)) {console.log('Payoff V3 already downloaded.');process.exit(0);}
...
if(!state) {
  const previous=await read(project+'payoff-clean-job.json');
  ...
  await fs.writeFile(project+name+'-request.json',JSON.stringify({model:body.model,source:'Existing provider-hosted original payoff; text-only correction; no local artwork or audio uploads',prompt,response_format:body.response_format},null,2));
...
console.log('Payoff V3: '+state.status);
...
console.log('Payoff V3 downloaded for local review.');
