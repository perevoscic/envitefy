...
const file=`public/projects/john-space-disco/vo-${mode}.mp3`,record=`projects/john-space-disco/voice-${mode}-request.json`;
...
const body={text:texts[mode],model_id:'eleven_multilingual_v2',voice_settings:{stability:0.40,similarity_boost:0.75,style:0.22,use_speaker_boost:true,speed:1.08}};
await fs.writeFile(record,JSON.stringify({status:'submitting',voice:'Jessica - Playful, Bright, Warm',body},null,2));
...
const result=await r.json();await fs.writeFile(file,Buffer.from(result.audio_base64,'base64'));await fs.writeFile(`projects/john-space-disco/voice-${mode}-alignment.json`,JSON.stringify(result.normalized_alignment||result.alignment,null,2));await fs.writeFile(record,JSON.stringify({status:'completed',voice:'Jessica',body},null,2));console.log(`Generated ${mode}`);
