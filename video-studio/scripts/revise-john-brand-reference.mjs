...
const b='public/projects/john-space-disco/',o='out/john-space-disco/',p='projects/john-space-disco/',voice='vo-create-jessica-reference-v1.mp3';
const review=JSON.parse(fs.readFileSync(p+'brand-jessica-reference-v1-audio-review.json','utf8'));
if(!(review.review.matchesStandard||review.review.matches_standard)||review.review.brand_ipa.replaceAll('/','')!=='ɛnˈvaɪtfaɪ'||review.review.syllable_count!==3)throw Error('Pronunciation review failed');
const voiceHash=crypto.createHash('sha256').update(fs.readFileSync(b+voice)).digest('hex');
if(voiceHash!==review.sha256)throw Error('Reviewed voice changed');
...
for(const r of revisions){
 ...
 records.push({...r,base,output,mix,voice:b+voice,voiceSha256:voiceHash,voiceDurationSeconds:duration,audioReview:p+'brand-jessica-reference-v1-audio-review.json',status:'exported-awaiting-final-checks'});console.log('Exported '+output);
}
fs.writeFileSync(p+'brand-standard-revision.json',JSON.stringify({date:'2026-09-09',standard:'../ENVITEFY_BRAND.md',pronunciation:'en-VITE-fy',ipa:'ɛnˈvaɪtfaɪ',records},null,2)+'\n');
