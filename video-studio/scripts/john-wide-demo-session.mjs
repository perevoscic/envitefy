...
export async function demoSession({view='card',record=false,width=600,height=860}={}){
 ...
 await context.route('**/*',async route=>{
  ...
  if(u.pathname==='/__john.js')return route.fulfill({path:out+'/wide-fixture.js',contentType:'application/javascript'});
  if(u.pathname==='/__john.css')return route.fulfill({path:out+'/vertical-app.css',contentType:'text/css'});
  if(u.pathname==='/john-card.webp')return route.fulfill({path:a+'/card-art-vertical.webp',contentType:'image/webp'});
  if(u.pathname==='/chat'||u.pathname.startsWith('/card/john-is-10')){
   let html=await fs.readFile(out+'/wide-fixture.html','utf8');
