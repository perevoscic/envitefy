...
const source='public/projects/john-space-disco/vo-create-v8.mp3';
const resultPath='projects/john-space-disco/brand-standard-voice-v8-audio-review.json';
const requestPath='projects/john-space-disco/brand-standard-voice-v8-audio-review-request.json';
...
if(sha256!=='f703e966a83d485d8f583ced91e1119942de84311c4a77db209e657292b0a757')throw Error('The verified synthetic stock-voice file changed.');
const tts=JSON.parse(await fs.readFile('projects/john-space-disco/voice-create-v8-request.json','utf8'));
if(tts.status!=='completed'||tts.voice!=='Jessica'||tts.body.text!=='Bring their birthday ideas to life with <phoneme alphabet="cmu-arpabet" ph="EH0 N V AY1 T F AY0">Envitefy</phoneme> Concierge.')throw Error('Synthetic narration provenance mismatch.');
const model='gemini-3.5-flash';
const destination='https://generativelanguage.googleapis.com/v1beta/models/'+model+':generateContent';
const prompt='Listen to the attached 2.9-second synthetic adult narrator audio. First transcribe the words you actually hear and report the audible pronunciation of the brand before Concierge in IPA and plain syllables. Then judge whether it matches this written standard: en-VITE-fy, /ɛnˈvaɪtfaɪ/, short en as in enter, stressed VITE rhyming with bite, final fy rhyming with fly. It must be a continuous THREE-syllable word; there must be no extra ee/ih/tee syllable between VITE and fy and no unnatural pause. Do not assume the sound matches the target. Report initial vowel, syllable count, strongest syllable, extra vowel or pause if any, complete intelligible sentence, natural delivery, any defect or uncertainty, and matchesStandard as a boolean. Base your judgment on the audio, not on the expected spelling. Return concise JSON.';
const provenance={source,sha256,bytes:bytes.length,scope:'Only a freshly generated stock Jessica adult TTS voice saying the user-provided promotional sentence. No personal recordings, child/parent dialogue, soundtrack, customer data or video included.',model,destination,prompt};
await fs.writeFile(requestPath,JSON.stringify({...provenance,status:'submitting'},null,2)+'\n');
const response=await fetch(destination,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':process.env.GEMINI_API_KEY},body:JSON.stringify({contents:[{parts:[{inlineData:{mimeType:'audio/mpeg',data:bytes.toString('base64')}},{text:prompt}]}],generationConfig:{temperature:0,responseMimeType:'application/json'}}),signal:AbortSignal.timeout(60000)});
