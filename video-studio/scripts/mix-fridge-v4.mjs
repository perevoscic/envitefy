...
const shutterTimes=[(timing.mainStartFrame+captureFrame)/timing.fps,(timing.cleanupStartFrame+captureFrame-timing.cleanupTrimStartFrame)/timing.fps];
...
function ff(args){const r=spawnSync('ffmpeg',['-y','-hide_banner','-loglevel','error',...args],{encoding:'utf8'});if(r.status)throw Error(r.stderr);}
 ...
 for(const t of shutterTimes)shutter(t);
...
ff(['-i',base+'music.mp3','-i',base+'v2-opening-edit.mp4','-i',base+'v2-ending-edit.mp4','-i',base+'v4-effects.wav','-filter_complex',mix,'-map','[a]','-ar','48000','-ac','2','-c:a','pcm_s16le',base+'v4-final-mix.wav']);console.log('V4 audio synchronized.');
