...
model=WhisperModel('small.en',device='cpu',compute_type='int8',download_root=str(root/'out/wedding-characters/local-review-model'),local_files_only=True)
for name in [arg for arg in sys.argv[1:] if arg != '--no-words']:
 source=root/'out/mom-just-snap-it'/name
 ...
 (root/'projects/mom-just-snap-it'/f'{source.stem}-small-transcript.json').write_text(json.dumps(result,indent=2))
