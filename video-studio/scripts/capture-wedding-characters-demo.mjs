...
const project='projects/wedding-characters', out='public/projects/wedding-characters';
await fs.mkdir('out/wedding-characters/capture',{recursive:true});
const browser=await chromium.launch({headless:true});
try {
 const context=await browser.newContext({viewport:{width:430,height:760},deviceScaleFactor:1,permissions:['clipboard-read','clipboard-write'],recordVideo:{dir:'out/wedding-characters/capture',size:{width:430,height:760}}});
 ...
 html=html.replaceAll(marker,marker+',\\"eventId\\":\\"demo-wedding-characters\\",\\"rsvpEnabled\\":true,\\"rsvpMode\\":\\"envitefy\\"');
 ...
 await context.route('**/*',async route=>{
  ...
  if(url.pathname==='/api/events/demo-wedding-characters/rsvp'){
 ...
 await page.waitForTimeout(1800);const consent=page.getByRole('button',{name:'Essential only',exact:true});if(await consent.isVisible())await consent.click();
 ...
 await page.getByPlaceholder('Name',{exact:true}).fill('Jordan Ellis');await page.getByPlaceholder('Email',{exact:true}).fill('wedding-guest@envitefy.com');
