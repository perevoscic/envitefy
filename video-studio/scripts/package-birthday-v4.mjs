...
const project='projects/birthday-second-job',file='out/birthday-second-job/birthday-second-job-9x16-v4.mp4';
...
run('ffmpeg',['-y','-hide_banner','-loglevel','error','-i','out/birthday-second-job/birthday-second-job-9x16-v4-render.mp4','-map','0:v','-map','0:a','-c:v','copy','-c:a','aac','-b:a','256k','-af','atrim=0:30','-t','30','-movflags','+faststart',file]);
...
run('ffmpeg',['-y','-hide_banner','-loglevel','error','-i',file,'-vf',"select='eq(n,0)+eq(n,90)+eq(n,250)+eq(n,375)+eq(n,560)+eq(n,599)+eq(n,600)+eq(n,659)+eq(n,660)+eq(n,661)+eq(n,750)+eq(n,761)+eq(n,762)+eq(n,767)+eq(n,771)+eq(n,840)',scale=180:320,tile=4x4",'-frames:v','1','out/birthday-second-job/birthday-v4-final-proof.jpg']);
