...
const p='projects/john-space-disco/',data=JSON.parse(fs.readFileSync(p+'share-narration-revision.json','utf8')),reports=[];
...
for(const r of data.records){
 ...
 const report={aspectRatio:r.aspectRatio,path:r.output,base:r.base,probe,videoStreamUnchanged:true,videoPacketSha256:videoHash,fullDecodePassed:true,bytes:bytes.length,sha256:crypto.createHash('sha256').update(bytes).digest('hex'),loudness,voice:r.voice,voiceSha256:r.voiceSha256,voiceStartSeconds:r.start,voiceDurationSeconds:r.voiceDurationSeconds,pronunciationAudioReview:r.audioReview,approvedPronunciationPreserved:r.voiceSha256==='998e444775b292b467c75607e44750f5ae482d4a430632f50986480dd5ce7edc',shareVoice:r.shareVoice,shareVoiceSha256:r.shareVoiceSha256,shareStartSeconds:r.shareStartSeconds,shareDurationSeconds:r.shareDurationSeconds};
...
fs.writeFileSync(p+'share-narration-verification.json',JSON.stringify({date:new Date().toISOString(),reports},null,2)+'\n');
