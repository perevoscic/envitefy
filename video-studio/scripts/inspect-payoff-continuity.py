...
Path('out/birthday-second-job').mkdir(parents=True, exist_ok=True)
...
for i in range(308,315):cv2.imwrite('out/birthday-second-job/payoff-source-'+str(i)+'.jpg',frames[i])
...
cv2.imwrite('out/birthday-second-job/payoff-alignment-preview.jpg',np.concatenate([frames[310],cv2.warpAffine(frames[311],mat,(720,1280))],axis=1))
