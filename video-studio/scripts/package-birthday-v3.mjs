...
const project='projects/birthday-second-job',file='out/birthday-second-job-9x16-v3.mp4';
...
run('ffmpeg',['-y','-hide_banner','-loglevel','error','-i','out/birthday-second-job-9x16-v3-render.mp4','-map','0:v','-map','0:a','-c:v','copy','-c:a','aac','-b:a','256k','-af','atrim=0:30','-t','30','-movflags','+faststart',file]);
...
run('ffmpeg',['-y','-hide_banner','-loglevel','error','-i',file,'-vf',"select='eq(n,0)+eq(n,60)+eq(n,250)+eq(n,330)+eq(n,357)+eq(n,375)+eq(n,395)+eq(n,560)+eq(n,689)+eq(n,691)+eq(n,800)+eq(n,840)',scale=216:384,tile=4x3",'-frames:v','1','out/birthday-v3-final-proof.jpg']);
