...
const project=path.join(studio,'projects/small-shower');
const out=path.join(studio,'public/projects/small-shower');
...
async function generate(mode){
 if(!['opening','trunk','create','guests','reveal','music'].includes(mode))throw new Error('Invalid shot');
 ...
 if(!state){
  ...
  if(['trunk','create','reveal'].includes(mode)){const prev=await readJson(path.join(project,'opening-job.json'));if(prev?.status!=='completed'){console.log('waiting for opening reference');return;}body.previous_interaction_id=prev.id;}
