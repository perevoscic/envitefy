...
const project='projects/birthday-second-job',file='out/birthday-second-job-9x16-v2.mp4';
...
run('ffmpeg',['-y','-hide_banner','-loglevel','error','-i','out/birthday-second-job-9x16-v2-render.mp4','-map','0:v','-map','0:a','-c:v','copy','-c:a','aac','-b:a','256k','-af','atrim=0:30','-t','30','-movflags','+faststart',file]);
...
run('ffmpeg',['-y','-hide_banner','-loglevel','error','-i',file,'-vf',"select='eq(n,330)+eq(n,331)+eq(n,332)+eq(n,333)+eq(n,350)+eq(n,357)+eq(n,395)+eq(n,554)',scale=216:384,tile=4x2",'-frames:v','1','out/birthday-v2-final-proof.jpg']);
