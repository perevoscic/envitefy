...
const entries=[];const cuts=[0,1,2,3,208,209,210,211,310,311,312,313,479,480,481,521,522,563,564,581,582,605,606,619,620,634,635,666,667,698,699,779,780,781,928,929,930,931,1048,1049];
for(const [format,w,h] of [['16x9',1920,1080],['9x16',1080,1920]]){
 const file=out+`/envitefy-snap-anime-${format}.mp4`;
 ...
 const select=cuts.map(f=>'eq(n,'+f+')').join('+');run('ffmpeg',['-y','-hide_banner','-loglevel','error','-i',file,'-vf',`select='${select}',scale=${format==='16x9'?384:216}:-1,tile=4x10`,'-frames:v','1',out+`/final-boundaries-${format}.jpg`]);
...
await fs.writeFile(out+'/final-loudness.txt',loudness);
...
await fs.writeFile(p+'/export-verification.json',JSON.stringify(verification,null,2));
await fs.writeFile(p+'/deliverables.json',JSON.stringify({campaign:'mom-just-snap-it',version:1,status:'rendered; technical and visual checks passed; pronunciation comparison pending',userApproved:false,pronunciationVerified:false,sharedSourceComponent:'src/mom-just-snap-it/SnapAnime.tsx',sharedAudio:'public/projects/mom-just-snap-it/final-mix.wav',editableProject:'out/mom-just-snap-it/envitefy-snap-anime-editable.zip',exports:entries},null,2));
