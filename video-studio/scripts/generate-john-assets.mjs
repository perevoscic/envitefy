...
async function generate(mode){
 ...
 if(!state){
  ...
  if(['create','update','payoff'].includes(mode)){const prev=await readJson(path.join(project,'hook-job.json'));if(prev?.status!=='completed'){console.log('payoff: waiting for hook reference');return;}body.previous_interaction_id=prev.id;}
