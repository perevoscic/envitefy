...
for(const name of process.argv.slice(2)){
 const data=await fs.readFile(`public/projects/birthday-second-job/${name}.mp4`);
 ...
 await fs.writeFile(`projects/birthday-second-job/${name}-media-review.json`,text);console.log(name,text);
}
