...
const share='vo-share-v1.mp3',shareStart=12.25;
const shareHash=crypto.createHash('sha256').update(fs.readFileSync(b+share)).digest('hex');
const shareDuration=Number(execFileSync('ffprobe',['-v','error','-show_entries','format=duration','-of','default=nw=1:nk=1',b+share],{encoding:'utf8'}).trim());
if(shareDuration<1.5||shareStart+shareDuration>18)throw Error('Sharing sentence timing needs review');
const releaseStart=shareStart+shareDuration+.08,releaseEnd=releaseStart+.25;
const duck="if(lt(t,12.05),1,if(lt(t,12.25),1-0.66*(t-12.05)/0.2,if(lt(t,"+releaseStart+"),0.34,if(lt(t,"+releaseEnd+"),0.34+0.66*(t-"+releaseStart+")/0.25,1))))";
const records=[];
for(const r of revisions){
 ...
 r.filter=r.filter.replace('[m];',",volume='"+duck+"':eval=frame[m];")
 .replace('[m][h][u][p][c][v][s]amix=inputs=7','[7:a]volume=1.1,adelay=12250|12250[n];[m][h][u][p][c][v][s][n]amix=inputs=8');
 if(!r.filter.includes('[7:a]'))throw Error('Sharing narration not inserted');
 ff(['-i',b+'music.mp3','-i',b+r.sources[0],'-i',b+r.sources[1],'-i',b+r.sources[2],'-i',b+voice,'-i',b+'vo-update.mp3','-i',b+r.sources[3],'-i',b+share,'-filter_complex',r.filter,'-map','[a]','-ar','48000','-ac','2','-c:a','pcm_s16le',mix]);
 ...
 ff(['-ss','12','-i',output,'-t','5','-vn','-ac','1','-ar','16000',o+r.prefix+'-v'+r.version+'-share-review.wav']);
 ...
 records.push({...r,base,output,mix,voice:b+voice,voiceSha256:voiceHash,voiceDurationSeconds:duration,shareVoice:b+share,shareVoiceSha256:shareHash,shareText:'One easy link. All the party details.',shareStartSeconds:shareStart,shareDurationSeconds:shareDuration,audioReview:p+'user-reference-narration-comparison.json',status:'exported-awaiting-final-checks'});console.log('Exported '+output);
}
fs.writeFileSync(p+'share-narration-revision.json',JSON.stringify({date:'2026-09-09',standard:'../ENVITEFY_BRAND.md',approvedPronunciationManifest:'assets/brand/audio/pronunciation.json',pronunciation:'in-VY-tih-fy',ipa:'ɪnˈvaɪtɪfaɪ',records},null,2)+'\n');
