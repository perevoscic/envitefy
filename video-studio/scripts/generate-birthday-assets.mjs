...
const project=path.join(studio,'projects/birthday-second-job');
const out=path.join(studio,'public/projects/birthday-second-job');
...
async function generate(mode){
 ...
 if(!state){
  ...
  if(mode==='payoff'){const prev=await readJson(path.join(project,'chaos-job.json'));if(prev?.status!=='completed'){console.log('payoff: waiting for chaos reference');return;}body.previous_interaction_id=prev.id;}
