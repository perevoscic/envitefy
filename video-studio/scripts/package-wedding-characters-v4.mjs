...
const dir='out/wedding-characters/',output=dir+'wedding-characters-9x16-v4.mp4';
function run(command,args){const r=spawnSync(command,args,{encoding:'utf8'});if(r.status!==0)throw new Error(r.stderr);return r;}
run('ffmpeg',['-y','-hide_banner','-loglevel','error','-i',dir+'wedding-characters-9x16-v4-final-render.mp4','-i','public/projects/wedding-characters/final-mix-v3.wav','-map','0:v:0','-map','1:a:0','-t','25','-vf','scale=in_range=pc:out_range=tv:out_color_matrix=bt709,format=yuv420p','-c:v','libx264','-preset','slow','-crf','17','-color_primaries','bt709','-color_trc','bt709','-colorspace','bt709','-c:a','aac','-b:a','192k','-ar','48000','-movflags','+faststart',output]);
...
fs.writeFileSync(dir+'v4-technical-review.json',JSON.stringify(result,null,2));
