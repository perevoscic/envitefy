...
for(const name of process.argv.slice(2)){
 const data=await fs.readFile(`public/projects/wedding-200-texts/${name}.mp4`);
 ...
 await fs.writeFile(`projects/wedding-200-texts/${name}-media-review.json`,text);console.log(name,text);
}
