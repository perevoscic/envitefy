...
async function checkSignups() {
  ...
  try {
    ...
    if (signupFormRes.rows.length > 0) {
      ...
      console.log(`Confirmed: ${responses.filter(r => r.status === 'confirmed').length}`);
      console.log(`Waitlisted: ${responses.filter(r => r.status === 'waitlisted').length}`);
      console.log(`Cancelled: ${responses.filter(r => r.status === 'cancelled').length}`);
