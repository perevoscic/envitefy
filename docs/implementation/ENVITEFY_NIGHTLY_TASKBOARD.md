...
- Reminder dispatch jobs. Reminder records are created as drafts; sending/list/cancel provider interfaces are not wired yet.
...
- OCR/source document UI and reminder dispatch remain later phases.
...
1. Add reminder provider interface plus a dry-run queue view before enabling delivery.
2. Add richer host dashboard cards for RSVP Board 2.0, response export, volunteer unclaim/edit, and payment exports.
3. Add source document ingestion for pasted text, upload, OCR review cards, and approval-to-schedule.
4. Add schedule hub agenda/calendar/board views backed by `event_occurrences`.
